...
TEST_F(TimerTest, RemoveExpiredWeakPtrDirectlyAfterCallbackTest)
{
    ...
    timer1->start(500ms);
    EXPECT_CALL(timerCallback1, Call()).WillOnce(Invoke([&]() {
        timer1 = nullptr;
    }));
...
TEST_F(TimerTest, PollDuringDuringPollDoesNotDirsturbBehaviorTest)
{
    ...
    timer1->start(500ms);
    EXPECT_CALL(timerCallback1, Call()).Times(4).WillRepeatedly(Invoke([&]() {
        uut->poll();
    }));
...
{
    EXPECT_EQ("300ns", testing::PrintToString(300ns));
    EXPECT_EQ("400us", testing::PrintToString(400us));
    EXPECT_EQ("500ms", testing::PrintToString(500ms));
    EXPECT_EQ("600s", testing::PrintToString(600s));
    EXPECT_EQ("700min", testing::PrintToString(700min));
    EXPECT_EQ("800h", testing::PrintToString(800h));
}
